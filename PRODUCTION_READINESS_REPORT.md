...
### Phase 1 — Money safety 🔶 IN PROGRESS (1 done, 4 not started)
...
| 7 | Real withdrawal tx construction (wallet-management) | ❌ Not started | `buildUnsignedTx` still returns the fake `[]byte("unsigned:%s:...")` string. No RLP/PSBT/Solana message construction. UTXO outpoints still not persisted on the withdrawal row. EVM nonce rollback still a no-op. Phase 0 did wire the real gRPC clients (`clients.NewMPCSigningClient`/`NewGatewayClient`) in prod mode, so the *signer* is real — but the unsigned-tx construction it's asked to sign is still fake. |
...
The stack now **fails fast in production mode** instead of silently falling back to stubs — the single biggest systemic pattern defect is fixed. The audit pipeline produces and consumes end-to-end on `audit.v1`. Three money-moving services (payment-orchestration, rail-connectors, exchange-connectors) now persist state to Postgres; blockchain-gateway's existing Postgres store is wired. The orchestrator dials real partner URLs (and will surface the remaining contract mismatches as runtime errors — the next step is regenerating consumers from `contracts/`). The custody core (P1.6) is now resolved: three real custody-provider adapters are implemented and tested against wiremock; signing up for any provider's free sandbox and setting `CUSTODY_PROVIDER` + credentials in compose is the only remaining step to exercise a real end-to-end signing flow.

What remains: **four Phase 1 items**. P1.7 (real withdrawal tx construction) is the next highest-stakes gap — the MPC signer is now real but the unsigned-tx it's asked to sign is still a fake string. P1.8 (reorg re-broadcast), P1.9 (ledger source of truth + HASH_CHAIN_SALT + unwrap removal), and P1.10 (`float64` elimination across 6 services) follow. Phase 2 (reliability/security) and Phase 3 (release/ops) are largely greenfield, with two partials (mTLS scaffolding in payment-orchestration; Chainalysis/TRM already wired in aml-kyt).

**Revised estimated time to production-readiness: 5–8 weeks** (down from 6–10), with P1.6 done. Critical path: P1.7 withdrawal tx construction (1-2 weeks) → P1.9 ledger source of truth (1 week) → P1.10 money standardization (1-2 weeks, parallelizable) → P2.11-15 reliability/security (2-3 weeks, parallelizable) → P3.16-20 release/ops (1-2 weeks). Sandbox integration testing of the custody adapters can happen in parallel with P1.7.
