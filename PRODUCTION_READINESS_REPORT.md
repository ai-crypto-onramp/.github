...
## Phase 0 Completion Re-Evaluation (2026-07-21)
...
### Phase 1 — Money safety ⏳ NOT STARTED (with caveats)

| # | Item | Status | Notes |
|---|---|---|---|
| 6 | Replace MPC in-house engine | 🔶 Partial | Custody-provider adapters (Fireblocks/Dfns/Turnkey) exist behind cargo features (`src/engine/{fireblocks,dfns,turnkey}.rs`) and `CUSTODY_PROVIDER` env selects them, but the default is still the in-house threshold engine that reconstructs the full private key. Decision required: ship with a custody provider (recommended) vs. audited from-scratch MPC crate. |
| 7 | Real withdrawal tx construction (wallet-management) | ❌ Not started | `buildUnsignedTx` still returns the fake `[]byte("unsigned:%s:...")` string. No RLP/PSBT/Solana message construction. UTXO outpoints still not persisted on the withdrawal row. EVM nonce rollback still a no-op. |
| 8 | Reorg re-broadcast (blockchain-gateway) | ❌ Not started | `reorg/detector.go:4` comment claims re-broadcast but `detector.go:89` only calls `Transition(..., chain.StatusReorgedOut, nil)` — no re-broadcast. Prepayment still doesn't wait for funding confirmation. REST contract with wallet-management still mismatched. |
| 9 | Ledger Postgres source of truth | 🔶 Partial | `MIGRATION_SET_SERIALIZABLE` is applied (`store.rs:18,118`) and config defaults to `SERIALIZABLE` (`config.rs:145`), but in-memory `Arc<Mutex<LedgerState>>` is still the read path. `HASH_CHAIN_SALT` is read from env (`config.rs:68`) but not mixed into `compute_hash`. `unwrap()` still on money path (`store.rs:465,531,532`). Audit emission now Kafka `audit.v1` (done in Phase 0). DB immutability trigger still removed. |
| 10 | Standardize money on int64/decimal | ❌ Not started | 471 `float64` references still across payment-orchestration, rail-connectors, pricing-quote, fx-hedging, liquidity-routing, treasury-orchestration. wallet-management still int64 (overflow risk). blockchain-gateway BTC balance still `uint64(v * 1e8)`. |

### Phase 2 — Reliability & security ⏳ NOT STARTED
...
| 13 | Notification real providers | ❌ Not started | `package.json` still has no real provider SDKs (no aws-sdk-client-ses/sns, twilio, firebase-admin). All 5 providers still `Stub*` in `providers.ts`. Outbound webhook still synthesizes `DELIVERED` without HTTP POST. |
...
| 17 | CVE scanning in CI | ❌ Not started | No govulncheck/npm audit/pip-audit/cargo-audit/Trivy in any workflow. |
...
| 20 | Runbooks + ADRs | ❌ Not started | Only `mpc-signing-service/docs/runbooks/*`. No ADRs. `.github/README.md` async diagram still inaccurate. |

### Revised headline verdict

The stack now **fails fast in production mode** instead of silently falling back to stubs — the single biggest systemic pattern defect is fixed. The audit pipeline produces and consumes end-to-end on `audit.v1`. Three money-moving services (payment-orchestration, rail-connectors, exchange-connectors) now persist state to Postgres; blockchain-gateway's existing Postgres store is wired. The orchestrator dials real partner URLs (and will surface the remaining contract mismatches as runtime errors — the next step is regenerating consumers from `contracts/`).

What remains: **Phase 1 (money safety) is the next priority**. The custody core (MPC), withdrawal tx construction, reorg re-broadcast, and ledger-source-of-truth are the highest-stakes gaps. The `float64` money standardization is a sweep across 6 services. Phase 2 (reliability/security) and Phase 3 (release/ops) are largely greenfield.

**Revised estimated time to production-readiness: 6–10 weeks** (down from 8–12), assuming Phase 1 starts with the custody-provider delegation path (Fireblocks/Dfns adapters already exist behind feature flags) and the consumer-side proto regeneration from `contracts/` lands early.
